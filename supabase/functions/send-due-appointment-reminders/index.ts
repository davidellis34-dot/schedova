...
async function getMessageCreditsRemaining(
  ...
  if (error) {
    console.error("reminder message credit lookup failed", {
      userId,
      error,
    });
    throw error;
...
async function consumeMessageCreditForReminder({
  serviceClient,
  userId,
  appointmentId,
  smsMessageLogId,
  ...
  const { data, error } = await serviceClient.rpc(
    "consume_message_credit_for_sms",
    {
      ...
      p_metadata: { message_type: MESSAGE_TYPE, source: "reminder_worker" },
    },
  );

  if (error) {
    console.error("reminder message credit deduction failed", {
      userId,
      appointmentId,
      smsMessageLogId,
      providerMessageId,
...
Deno.serve(async (req: Request) => {
  ...
  for (const smsSettings of (smsSettingsRows || []) as SmsSettingsRow[]) {
    ...
    if (!isPaid && messageCreditLookupError) {
      console.error("reminder worker message credit lookup blocked send", {
        userId,
        error: serializeDetails(messageCreditLookupError),
      });
      failed += 1;
      continue;
    }

    if (!isPaid && messageCreditsRemaining <= 0) {
      console.log("reminder worker skipped user without Pro or credits", {
        userId,
    ...
    for (const appointment of (appointmentRows || []) as AppointmentRow[]) {
      ...
      if (!isPaid && messageCreditsRemaining <= 0) {
        console.log("reminder worker skipped due appointment without credits", {
          userId,
          appointmentId,
          messageCreditsRemaining,
        });
      ...
      try {
        ...
        if (messageCreditsRemaining > 0) {
          try {
            messageCreditsRemaining = await consumeMessageCreditForReminder({
              serviceClient,
              userId,
              appointmentId,
            ...
            console.error("reminder sent but credit was not deducted", {
              userId,
              appointmentId,
              queuedLogId,
              providerMessageId,
