...
export default function OnboardingScreen() {
  ...
  async function saveService() {
    ...
    try {
      ...
      if (result.error || !result.data?.id) {
        throw result.error || new Error("Service could not be saved.");
      }
  ...
  async function saveClient() {
    ...
    try {
      ...
      if (result.error || !result.data?.id) {
        throw result.error || new Error("Client could not be saved.");
      }
  ...
  function openFirstAppointment() {
    if (!savedClient?.id || !savedService?.id) {
      Alert.alert(
        "Add a service and client first",
        "You can book an appointment once those two steps are complete.",
      );
      return;
    }
  ...
  return (
    ...
    >
      <ScreenHeader
        ...
        subtitle="A few quick steps to get your first appointment booked."
      />
      ...
      <Text style={{ color: colors.mutedText, fontWeight: "800", marginBottom: 10 }}>
        {progress} · {STEPS[step]}
      </Text>
      <AppCard style={{ gap: 16 }}>
        ...
        {step === 1 ? (
          <>
            ...
            <Text style={{ color: colors.mutedText, lineHeight: 21 }}>
              You can add more services any time.
            </Text>
            <AppTextInput
              label="Service name"
              value={serviceName}
        ...
        {step === 2 ? (
          <>
            ...
            <Text style={{ color: colors.mutedText, lineHeight: 21 }}>
              A phone number is optional. Add it later before texting this client.
            </Text>
            <AppTextInput
              label="Client name"
              value={clientName}
        ...
        {step === 3 ? (
          <>
            ...
            <Text style={{ color: colors.mutedText, lineHeight: 21 }}>
              We’ll preselect {savedClient?.name || "your client"} and{" "}
              {savedService?.name || "your service"}. Choose the date and time,
              then save.
            </Text>
            <AppButton
              title="Book first appointment"
              onPress={openFirstAppointment}
              disabled={!savedClient || !savedService}
            />
            <Pressable
        ...
        {step === 4 ? (
          <>
            ...
            <Text style={{ color: colors.mutedText, lineHeight: 21 }}>
              SMS is optional. Review the setup whenever you are ready to send
              appointment texts. You can always return here from Settings.
            </Text>
            <AppButton
              title="Review SMS Settings"
              variant="secondary"
              onPress={() => router.push("/settings/sms" as any)}
            />
            <AppButton
              title="Continue"
              onPress={() => void updateDraft({ step: 5 })}
            />
            <Pressable
              onPress={skipCurrentStep}
              accessibilityRole="button"
        ...
        {step === 5 ? (
          <>
            ...
            <Text style={{ color: colors.mutedText, lineHeight: 22 }}>
              Your dashboard will keep your bookings, clients, services, and
